 ...
 * choke point for observability, context propagation, and ledger recording (5334.md §2.8); a thrown exception surfaces
 * as a failed `CompletionStage` because the runtime's own dispatch already normalizes synchronous throws into a failed
 * `Future` -- this class deliberately does not duplicate that translation.
 */
@InternalApi private[javasdk] final class ClassifierProvider(
   ...
   * available yet. This is a known v1 correlation gap (5334.md Phase 3): those classifications ride a root OTel context
   * rather than the caller's, to be revisited once guardrails carry their own classifier-invocation path (tracked
   * alongside the async-guardrail migration, #5383).
   */
...
@InternalApi private[javasdk] object ClassifierProvider {
  ...
  private def fromClassification(c: Classification): SpiClassifier.Result =
    new SpiClassifier.Result(
      ...
      c.metadata.asScala.toMap)

  /**
   * Wraps a user classifier so the runtime can invoke it once registered (5334.md §2.8). `resolve` is called on every
   * invocation rather than once at adapter-construction time, so registration (`spiConfiguredClassifiers`) can happen
   * before the classifier itself is safe to construct (see the scaladoc there); `getOrCreate` memoizes, so this is
   * cheap after the first real resolution. Only [[SpiClassifier.TextContent]] exists in v1; the content's
   * `telemetryContext` is not yet surfaced to the user's classifier (exposing it, e.g. a `Tracing` handle mirroring
   * `ModelGuardrail.CallContext.tracing()`, is a parked follow-up -- the public `classify(String)` has no per-call
