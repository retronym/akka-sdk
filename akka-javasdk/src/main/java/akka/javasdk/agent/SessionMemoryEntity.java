...
public final class SessionMemoryEntity extends EventSourcedEntity<State, Event> {
  ...
  public Effect<SessionHistoryResult> fetchHistory(GetHistoryCmd cmd) {
    ...
    //
    // The block-aligned window cannot take this shortcut. Its start is derived from the total
    // message count, so a partial in-memory view would place the start somewhere else than the
    // journal path does, and the prompt would change from one turn to the next — which is the whole
    // point of the window. Reconstruct from the journal instead.
